...
const PromptBox = ({isLoading, setIsLoading}) => {
  ...
  const sendPrompt = async (e) => {
    ...
    try{
      ...
      setChats((prevChats) => prevChats.map((chat) => CharacterData._id === selectedChat._id ? {
        ...chat,
        messages: [...chats.messages, userPrompt]
      } : chat))
      ...
      if(data.success){
        ...
        for(let i =0; i <messageTokens.length; i++){
          setTimeout(() => {assistantMessage.content = messageTokens.slice(0, i+1).join(' ');
            setSelectedChat((prev) => {
  ...
  return (
    <form onSubmit={sendPrompt} className={`w-full ${false ? 'max-w-3xl' : 'max-w-2xl'} bg-[#404045] p-4 rounded-3xl mt-4 transition-all`}>
        <textarea onKeyDown={handleKeyDown} onChange={(e) => setPrompt(e.target.value)} value={prompt} rows={2} placeholder="Message Voxa-AI" className="outline-none w-full resize-none overflow-hidden break-words bg-transparent" required />
